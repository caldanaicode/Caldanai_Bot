...
- **Per-missed-visit decay.** When a known passerby visits a player and the player doesn't interact at all that visit (no verbs, no witnessed kills), -1 credit applied on depart. Floors at COLD. Players who've never interacted with an NPC have no slice and don't accumulate decay until they have a relationship to lose. `flee_from_attack` skips the decay (`apply_decay=False`) so bystanders don't get punished when another player attacks the NPC.
