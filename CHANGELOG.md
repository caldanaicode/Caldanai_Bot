...
- **New `NAME_DROP_KEYWORDS: Dict[str, str] = {}` class attr** on `PasserbyPlugin` base (empty default → opt-in). Wren's entry: `{"marn": "Marn", "halrick": "Halrick"}` mapping case-insensitive player-side tokens to the case-sensitive pool-line substrings to weight.
- **Per-visit `heard_keywords: set` instance attr** initialized in `PasserbyPlugin.__init__`. Resets implicitly when the NPC departs and a fresh instance spawns for the next visit.
- **`overhear_keywords(game, message)` in `passersby/spawn.py`** scans the message content for any `NAME_DROP_KEYWORDS` key and accumulates matches on the NPC's `heard_keywords` set. Wired into the existing `on_message` listener in `rpg_social_commands.py` alongside the existing `overhear_mentions` call. No cue dispatched — the bias surfaces at departure, not at mention-time, so the mechanism stays texture rather than feedback-loop.
- **`_pick_departure_line(npc)`** factored out of `depart_passerby`. When `NAME_DROP_KEYWORDS` and `heard_keywords` are both non-empty AND at least one pool line contains a matching substring, the line is selected via `random.choices` with a 10× weight on matching lines (≈ 91% near-determinism on Wren's 1-keyword-line-of-6 pool). All other paths fall through to plain `choice` — empty pool, no keywords heard, no pool line matches any heard substring.
- **Tests.** `tests/test_passerby_spawn.py` gains `TestKeywordBiasedDeparture` (3 cases: empty-heard-uses-choice, heard-with-match-weights-correctly, heard-without-match-falls-through) and `TestOverhearKeywords` (10 cases: no-passerby, opt-out NPC, single token add, case-insensitive match, no-match no-op, duplicate-not-re-added, multiple-tokens-in-one-message, silhouette-NPC-overhears, missing-content-attr-defense, end-to-end-overhear-then-depart-biases). Full suite 4676 passed.

### 2026-05-14 — `$equip` migrated to the fuzzy-resolve dispatcher with call-site `@`-split parsing (Step 4 of 4)

Per-term resolution in `$equip` now flows through call-site `@`-split parsing + two `fuzzy_resolve` calls — one against `ItemConverter` for the item portion (pre-`@`), one against `EquipmentSlotConverter` for the placement hint (post-`@`). The dispatcher itself stays simple — one string, N target types, first match wins; per-verb parsing complexity (the `@`-split) lives at the call site per the burden-at-call-site principle. Player-visible behaviour unchanged for every existing `$equip` invocation: same multi-term grammar (`$equip wand.b short.b jerkin.s glove.1`), same auto-slot when no `@<hint>` is supplied, same legacy 2-arg trailing-hint form (`$equip sword left` → `sword@left` rewrite), same per-term partial-failure ("first item succeeds, second emits no-match line"), same fully-qualified-bypass-quality-gate displace for `<name>.<quality>.<index>` shapes, same identical-label first-match-wins collapse on ambiguity. The `@`-split form itself was already accepted by the previous `resolve_items_or_notify` helper — Step 4 doesn't add new syntax, it relocates the parsing to the call site so the same shape now flows through the dispatcher family. Three known `$equip` resolver UX bugs from `project_equip_resolver_ux.md` are explicitly out of scope and stay open.

- **New `ItemConverter` + `EquipmentSlotConverter`** in `caldanai/lib/rpg/helpers/converters.py`. `ItemConverter.try_convert` wraps a new `resolve_item(player, query)` helper in `helpers/resolvers.py` — thin wrapper around `Player.resolve_item_query(query, "equip")` returning the single matching Item or None on no-match / ambiguity / missing context (the dispatcher's "can't pick one" collapse). `EquipmentSlotConverter.try_convert` wraps a new `resolve_equipment_slot(hint)` helper that handles the short-vocabulary (`l` / `left` / `r` / `right`), full `part.key` reverse lookup (`head.worn`, `hand.left.held`), and bare-key fallback (`worn` / `held` when exactly one slot uses that key) — the same hint vocabulary the previous `resolve_items_or_notify` understood, extracted to a free function so it can plug into the converter shape. `_` (the "anywhere it fits" wildcard) returns None so the call site treats it as auto-route. Both converters preserve a `convert()` forward-compat seam.
- **`$equip` rewrite** in `caldanai/lib/cogs/rpg_inventory_commands.py`. The per-query loop now: `term.partition("@")` → `fuzzy_resolve(ctx, item_str, ItemConverter)` → on None re-query `player.resolve_item_query(item_str, "equip")` once to distinguish ambiguity from no-match and dispatch the precise "did you mean: ..." vs "don't seem to have" message → `fuzzy_resolve(ctx, hint_str, EquipmentSlotConverter)` for the placement portion → call site builds the final `(item, slot)` pair, applies the fully-qualified-displace heuristic, and calls `player.equip`. The two extra hops (call-site re-query on None for ambiguity surfacing) are the cost of preserving the existing UX through a dispatcher-Optional contract that collapses ambiguity and no-match into one None.
- **Tests.** New `tests/test_equip_dispatcher_migration.py` (10 cases) pins the end-to-end `equip.callback` against a real Player: `@`-split routes the item + hint independently, no-`@` auto-resolves the slot, multi-term invocations process each term independently, mixed `@`-and-no-`@` terms both resolve, the `no-item` and `unknown-slot` miss paths preserve their pre-migration wording, partial failures don't block other terms, an actually-ambiguous selector (`leather.fine` against three distinct leather items) surfaces the "did you mean" candidate list while a pure no-match emits the "don't seem to have" wording — together pinning both branches of the call-site re-query, and the legacy `$equip sword left` form rewrites through the `@`-split pipeline. `tests/test_converters.py` gains `TestItemConverterTryConvert` (7 cases) and `TestEquipmentSlotConverterTryConvert` (8 cases) covering single-match / no-match / ambiguity-collapse / convert-still-raises for ItemConverter, plus short-vocab / full-part-key / dotted-part / unknown / underscore / empty / convert-still-raises for EquipmentSlotConverter.

### 2026-05-14 — `$kill` part resolution migrated to the fuzzy-resolve dispatcher (Step 3 of 4)

Per-token body-part resolution in `$kill` (aliased `$attack`) now flows through `fuzzy_resolve(ctx, token, PartConverter)` instead of a direct call into `resolve_part(monster, token)`. Scope is parts-only per the Step 3 design clarification: multi-monster swarms aren't shipped, so a single spawned monster makes every `$kill`-target a body-part token. Monster-token peeling (`_strip_leading_monster_token` — the `$kill werewolf arm.left` → strip `werewolf`, target `arm.left` shape) is preserved intact; the dispatcher swap is per-token below the peel. `$target` (aliased `$aim`/`$focus`) shares the same `_parse_part_targets` helper and rides the migration for free. Player-visible behaviour unchanged: same fuzzy abbreviation resolution (`arm.l` → `arm.left`), same multi-match expansion (`leg` → both legs on a goblin, all four on a werewolf), same "No targetable part" warning on a no-match token followed by random-attack fallback, same bare-invocation shape (`$kill` alone still joins combat). When multi-monster ships, the call site picks up monster-token peeling through the dispatcher; the converter family stays unchanged.

- **`$kill` / `$target` rewire** in `caldanai/lib/cogs/rpg_user_commands.py`. `_parse_part_targets` flips from sync `(target_str, monster)` to async `(ctx, target_str)`, iterates tokens, and awaits `fuzzy_resolve(ctx, token, PartConverter)` per token. Both call sites (`attack`, `target_part`) get an `await`. The monster argument falls out — `PartConverter` reads `game.monster` through `RpgUtilities.get_game(ctx)`. Module-level imports add `fuzzy_resolve` and `PartConverter`. Stale `monster_token_consumed` unpacking on `_strip_leading_monster_token` flattens to `_` (the flag was never read).
- **Tests.** `tests/test_kill_command_grammar.py` gains a `TestKillDispatcherIntegration` class (4 cases) covering the dispatcher-pattern proofs: a fuzzy abbreviation (`$kill arm.l`) resolves through `PartConverter` to the canonical name, a token that matches every leg on a werewolf-shaped four-leg body expands to all four canonical leg names, a no-match token surfaces the existing warning and falls back to random-attack (combat_targets `None`), and bare `$kill` joins combat without walking the dispatcher. Existing 21 kill-grammar tests stay green after a fixture update that adds `RpgUtilities.get_game` to the patched-helpers set (called from inside `PartConverter` during the per-token walk). `tests/test_part_target_parsing.py` adopts the same patched-`get_game` shape — all 12 `_parse_part_targets` cases flipped to async and assert against the same canonicalisation / dedup / ambiguous-expansion / destroyed-part-skip surface as before.

### 2026-05-14 — `$haunt` migrated to the fuzzy-resolve dispatcher (Step 2 of 4)

Proof-of-pattern migration for the dispatcher scaffolding shipped in Step 1. `$haunt`'s 14-line inline cascade (mention-peel → `RpgUtilities.get_player`, `resolve_active_monster` for monsters, recursive `await self.haunt(ctx)` on miss) collapses to a single `fuzzy_resolve(ctx, target, CreatureConverter)` call. Bot-mention reject stays as a verb-specific call-site pre-check; the recursive fallback is flattened — a None result falls through to the no-target ambient pool inline. Player-visible behaviour unchanged: same fuzzy resolution against monster + player roster (now via `CreatureConverter` with default `prefer="monster"`), same silent fall-through on miss (no error message for unknown targets), same "figment of your imagination" reject for `$haunt @<bot>`, same ambient ghost flavor when no target is supplied. This is the canonical proof that the dispatcher's Optional-return shape supports silent-fallback UX cleanly. `$kill` (parts-only) and `$equip` (call-site `@`-split parsing) land on the same pattern in subsequent steps.

- **`$haunt` rewrite** in `caldanai/lib/cogs/rpg_social_commands.py`. Drops the inline `from ... import resolve_active_monster`, the recursive `await self.haunt(ctx)` fall-through, and the `isinstance(haunted, Creature)` type-check (the converter already returns Creature-or-None). Bot-mention pre-check stays — peeling the bot identity is verb-specific UX, not converter-layer concern. Module-level imports add `fuzzy_resolve` and `CreatureConverter`.
- **Tests.** `tests/test_haunt_target_match.py` gains a `TestHauntDispatcherIntegration` class (4 cases) covering the dispatcher-pattern proofs: silent fall-through on miss lands in the ambient ghost pool (not an error message), `<@id>` mentions route through the dispatcher to `PlayerConverter`'s mention fast-path, `$haunt @<bot>` still rejects at the call-site pre-check, and the default `prefer="monster"` direction wins on a monster/player name collision (cyclops + a "Cycelle" player resolves the cyclops). The existing 7 `$haunt` regression tests stay green after a fixture update that also patches `RpgUtilities.get_game` (called from inside the Converter family during the dispatcher walk, in addition to the entry-point `get_game_and_player`).
...
- **`Reach` axis is now load-bearing.** `Creature.get_trait_multiplier` and `get_hit_narration` take `source.reach` as an additional input. New per-creature `ranged_traits: Dict[DamageTypes, float]` overlay (instance dict, mirror of `traits`) and `RANGED_NARRATIONS: Dict[DamageTypes, str]` class attr (mirror of `HIT_NARRATIONS`, MRO-merged via `_resolved_ranged_narrations`) are consulted only when `source.reach == Reach.RANGED`. Merge wins on key collision (ranged entry shadows base entry), so a creature can author bow-specific flavor that fires only on ranged sources without touching its base damage-type narrations. `_matches_dmg_type` factors the COMBINED-gate logic into a single helper for both the trait walk and the narration deviation sort.
- **`DamageTypes.RANGED` removed from the enum.** Bit `1 << 1` is left unused (no renumbering — preserves byte-equivalence with legacy stored ints for the other bits). `ANY` composition drops RANGED. `__str__` strip-loop drops the RANGED branch. Emoji map and emoji ordering drop the RANGED entry. The `CombatRanges.RANGED` enum and `Reach.RANGED` enum are unrelated and unchanged.
- **`Weapon.REACH` class attr replaces damage-type derivation.** Bow / wand declare `REACH = Reach.RANGED` directly. `Weapon.skill` factors the reach word into the format string (`"one-handed " + ("ranged " if reach == RANGED else "") + damage_type.canonical`) — preserving the exact player-facing skill names ("one-handed ranged magical", "one-handed ranged piercing") byte-for-byte. Bow `damage_type = PIERCING`, wand `damage_type = MAGICAL` — the carrier-compound `RANGED | COMBINED` hack drops out.
- **Player skill-key migration (v2 → v3).** `_migrate_skill_keys` renames legacy `"one-handed ranged piercing combined"` → `"one-handed ranged piercing"` and the magical equivalent on next player load. Real elemental compound skills (torch: `"one-handed bludgeoning fire combined"`) are unaffected — the migration only fires on keys containing `"ranged"` whose damage-type word isn't a compound alias name. XP merges into the destination key on the (rare) collision case.
- **Per-monster trait + narration migrations.** toad / bearowl / dragon / giant moved their RANGED-keyed entries into `ranged_traits` / `RANGED_NARRATIONS` with the RANGED bit (and the carrier-COMBINED bit where it was the conflation hack) dropped from keys. Bow-specific bonuses become `ranged_traits[PIERCING] = 1.5` (toad, dragon — only fires when the source is ranged AND piercing); generic ranged-vulnerability becomes `ranged_traits[ANY] = 1.5` (bearowl — Vael's *1.5 wand observation preserved) or `ranged_traits[ANY] = 0.75` (giant — pebble-against-a-hill). Bandit's dead-weight `traits[RANGED] = 1.0` dropped entirely (was a no-op against the max-win aggregation). Hydra spit / hex actions drop the RANGED bit from their action `dmg_type` since the action already declares `"reach": Reach.RANGED` separately. ANY-set traits (`ANY - (RANGED | X)`) collapse to `ANY - X` with identical effective semantics.
- **MAGICAL gap fix (today's earlier work, preserved).** Eight monsters got new MAGICAL `HIT_NARRATIONS` entries during the audit pass — toad, bearowl, werewolf, minotaur, sheep, cyclops, plus skeleton + spirit via the Undead mixin. They survive the structural refactor unchanged (MAGICAL bit is what stays in `DamageTypes`).
- **Bow icon preserved across both display surfaces.** Two reach-prefix injections at the display layer cover what the conflated `DamageTypes.RANGED.emoji` used to do automatically: `Player.get_skill_display` prepends 🏹 when the skill key contains `"ranged"` (so `$skills` for wand mastery still reads "🏹✨ one-handed ranged magical"), and `AttackResult._reach_prefix_emoji` prepends 🏹 when `source.reach == Reach.RANGED` (so per-attack combat rows still read `🟢 Left → torso ... 🏹✨` for wand strikes). Single helper on the AttackResult class so the prefix logic lives in one place; consumed by both `to_display_parts` and `to_markdown`.
- **Tests.** `test_weapon.py` reach assertions updated to test `Bow().reach == Reach.RANGED` via the class attr (was: `weapon.damage_type & DamageTypes.RANGED`). `test_damage_type_aliases.py` canonical / round-trip tests updated for the new bow / wand damage_type shape (`PIERCING` / `MAGICAL` only). `test_dragon_body_parts.py` trait-identity assertion updated for the `ranged_traits[PIERCING]` migration. Render-proofed the nine MAGICAL lines via `tools.render_flavor` (one pronoun-mix caught and fixed on the sheep line).
...
- **`$help all` static-dump path.** Discord's API doesn't expose component interactions to bot accounts, which means automation reading help via `tail_channel` can see the View's current page but can't drive the Select / Prev / Next to enumerate everything. `$help all` renders every visible category + its commands as plain embeds with no View attached — one field per category (chunked by `(count — n/m)` when a category overflows Discord's 1024-char field-value cap, multi-embed when the 6000-char per-embed total would be crossed). Bot reads the full surface in a single invocation.
- **Tests** — new `tests/test_help_commands.py` (32 tests) covering label-token stripping, per-command line rendering, `HelpView` landing + category-page embeds, button state across pages, category resolution, can_run filtering, hidden-command filtering, empty-category dropping, `$help` routing through bare / category-name / command-name / `all` / missing paths, and `$help all` dump shape (one-field-per-category, chunk-on-overflow, multi-embed-on-total-overflow, no-categories-graceful).
...
- **`rpg_presence_commands.py` cog** — six verbs, each delegating to the unified `dispatch_expressive_verb`. Bare invocation hits per-verb self-directed pools (8 lines each, emotion-neutral per `project_verb_adverb_coloring.md` so the same $bite twice doesn't read as manic mood-swing). Mention path routes through `Player.handle_verb` → warmth-resolved narration. Token-target hits passerby / static-object reactions per the coverage matrix.
- **`$thank` in social cog** — WARM-default warmth-aware verb. Full intent + acceptance pools across 5 tiers, dead-invoker + dead-target pools, BotResponder scripted reply, `_BARE_NO_TARGET_TEMPLATES` + `_SELF_TARGET_LINES` entries. Aliased `$thanks` / `$ty`.
- **V2 warmth-aware presence narration** — `_NARRATION_POOLS` extended with 12 dicts (intent + acceptance for each presence verb × 5 tiers, ~210 authored lines total). Tier register layered with per-verb tone: $bite stays physical-fidget across all tiers (HOT = playful mock-snap, COLD = sharp nip); $tend / $sit can carry mild affection at HOT/WARM via physical specifics (knees-touching, working-a-knot-out); the four canonically emotion-neutral verbs ($lean / $rest / $ponder / $bite) avoid emotion adverbs entirely. Acceptance asymmetry honored — a WARM-intent lean meets independent COOL/COLD acceptance without echoing the actor's tier. Plus dead-invoker (24 lines) and dead-target (18 lines) pools per verb.
- **Per-passerby reactions** per the coverage matrix — wagoneer ($thank); wren ($thank/$ponder/$tend); herbalist ($thank/$rest/$ponder/$tend/$lean); shepherd ($thank/$sit/$rest/$lean). Each across 5 warmth tiers, in NPC voice (wagoneer working-class deflection, wren secondhand-aphorisms-from-Halrick, herbalist mythic-aphoristic, shepherd rural-patient). $bite intentionally has NO passerby reactions in V1 (Caels: *"we'll figure out how to let players be jackasses later"*).
- **Per-static-object branches** — campfire ($sit/$rest/$lean/$tend/$ponder/$bite × FireState LIT/EMBERS/OUT); stone field ($lean/$sit/$rest/$ponder/$tend × day/night). $bite intentionally omitted from stone field — stones don't invite the absurd the way the fire does.
- **`$lean @player` mention-path fix.** Initial ship had presence cog NOT extracting mentions; `$lean @Vael` rendered the bad-token miss line on the raw `<@id>` text. Fixed via dispatcher's new `with_target_template` kwarg (renders a per-verb "near @2" template when a Player target's `handle_verb` returns None) + per-verb extraction of `ctx.message.mentions[0]` in the cog wrapper. With V2 warmth-aware narration now in `_NARRATION_POOLS`, the with_target_template is a safety net rather than the primary path.

### 2026-05-07 — Verb-dispatch retrospective fixes + contract tests

Cleanup pass over commit `7a8ffd3` per the retrospective-review tasks. None of these are gameplay-visible regressions; they tighten correctness, kill dead code, and pin contract behavior with tests.

- **Monster "doesn't react" miss line restored** (was `7a8ffd3` regression). Pre-refactor, `$glare bandit` against a monster lacking that verb in `SOCIAL_REACTIONS` rendered `"@1Dc does not react to @2np glare."`; post-refactor the chain fell through to the bad-token miss. Restored as a final fallback in `Creature.on_social` — runs after SOCIAL_REACTIONS lookup and the hug delegation, gated to verbs in `warmth.SOCIAL_COMMANDS`. Subclass overrides (Bandit's high-five branch) pick it up by `return super().on_social(...)`. Same architectural location as pre-refactor; same wording.
- **`StaticObjectPlugin.handle_verb` kwargs forwarding** — was unpacking `kwargs.values()` as positional args, fragile because it relied on dict insertion order ($feed's `fuel_arg` happened to land as `args[0]`). Switched to `**kwargs` forwarding by name; campfire's `on_verb` signature updated to read `kwargs.get("fuel_arg")`. Stone field's `on_verb` signature aligned for parity. Three test calls updated from positional `None` to `fuel_arg=None`.
- **`PasserbyPlugin.handle_verb` defensive guard** returns `None` (fall-through) instead of `""` (silent swallow) when `channel_id` / `actor_id` is missing. Defensive case is rare; fall-through is the safer default.
- **`_GENERIC_BARE_FALLBACK` render-pipeline mismatch fixed, then deleted.** Was using `.format(name=...)` while `self_directed_pool` used `parse()` — every-other-pool-uses-parse-tokens landmine. Briefly unified on `parse()`; on closer inspection every entry was unreachable (presence cog passes `self_directed_pool` explicitly, $bow goes through `_dispatch_self_directed`, $thank through `_BARE_NO_TARGET_TEMPLATES`). Dict deleted; catch-all `*@1 <verb>s, vaguely.*` line is the sole bare-no-pool fallback.
- **`stone_field._on_listen` `@1m` → `@1d`.** Pre-existing one-line typo. `@1m` falls back to bare name "stone field"; `@1d` correctly renders "the stone field."
- **Dead code removed** — `resolve_verb_target` (unused since the dispatcher reimplemented its mention path inline) + its private helper `_player_from_mention`. Imports updated.
- **Contract tests** — new `tests/test_verb_dispatch_contract.py` (12 tests). Pins three-state return semantics (`None` / `""` / non-empty), `Creature.handle_verb` empty-string→None collapse, `StaticObjectPlugin.handle_verb` SUPPORTED_VERBS gate, kwargs-forwarding contract, and the new `Creature.on_social` bland-line fallback (4 tests covering known-warmth verb fires the line, unknown verb returns "", handle_verb passes the line through, SOCIAL_REACTIONS entry beats the bland line).

### 2026-05-07 — `tools/tail_channel` Gateway operational additions

Two flags for agents that run tail_channel inside the same session that's also posting via bot_player.

- **`--exclude-self`** — drops messages / edits / reactions authored by the Gateway client itself (the tester bot). Default off so external Monitors retain visibility of the tester bot's actions; agents whose Monitor is "themselves typing back" opt in. Module-level `_should_filter_self_event` helper compares by `.id` (not `is`), with 6 unit tests pinning the comparison contract — same lesson as the verb-dispatch refactor's `mention is bot_user` regression generalized.
- **`--debug-log <path>`** — appends per-event timing telemetry (Discord-emit timestamp, callback-entry, recv-to-print gap-ms) to a file. **File-only**; doesn't touch stdout (Monitor watches stdout) or stderr (Monitor's task output captures stderr) so the agent stays unaware. Used to diagnose Monitor-side queue lag — successfully exonerated tail_channel itself once Caels' Windows clock was synced and the recv-to-print gap stabilized at sub-millisecond.

### 2026-05-04 — Verb-dispatch unification: VerbResponder Protocol + cog-side helper

Three independent verb-handling shapes had grown across the codebase: `StaticObjectPlugin`'s `SUPPORTED_VERBS` + `on_verb`, `PasserbyPlugin`'s `SOCIAL_REACTIONS` dict + per-cog routing helpers, and `Creature`'s `on_social`. Each entity type was a special branch in every relevant cog: `if mention is bot.user → scripted reply`, then `if monster.matches → on_social`, then `if passerby.matches → _maybe_route_to_passerby_social`, then `if static_object.matches → on_verb`, then warmth path. New verbs meant editing every cog body. The refactor collapses the five branches into one resolver chain + one dispatch call.

- **`VerbResponder` Protocol** in `helpers/verbs.py` — every entity that can receive a verb implements `matches_token(token) -> bool` + `handle_verb(verb, game, actor, *, invocation, **kwargs) -> Optional[str]`. Three return states: `None` = "I don't claim this verb, fall through to next responder"; `""` = "consumed silent — I claimed it but produced no narration, do not fall through"; non-empty `str` = "render this." `walk_token_chain(game, token)` is the canonical resolver — yields candidates in fixed priority (passerby → silhouette → monster → static_object → player_fuzzy), each filterable per-call via `include_*` flags.
- **`dispatch_expressive_verb`** in `helpers/verb_dispatch.py` — the cog-side function that does the full pipeline: dead-invoker guard (opt-in via `dead_invoker_pool`), bot-mention path (with doppelganger-disguise check), token-chain walk, bare-no-target self-directed pool, bad-token italic miss. Cog command bodies become two lines: `await dispatch_expressive_verb(ctx, "hug", target_token=target)`. Lives in `helpers/` rather than `cogs/` so the auto-loader doesn't try to register it as a Discord cog with a `setup()`.
- **`BotResponder`** in `creatures/bot_responder.py` — the narrator's mention-receiver. Carries the per-verb scripted-reply dict for the warmth-aware verb set ($salute / $comfort / $poke / $nod / $glare / $shank / $tickle / $taunt / $wink / $wave / $greet) plus $hug's special HAL-gif file dispatch. Pre-refactor, the bot-mention case was a per-cog special branch repeated across every social command; lifting it to a real responder routes it through the same dispatch channel as everything else.
- **Migrated cogs.** `rpg_world_commands.py` ($light / $feed / $gaze / $touch / $listen) and the 15 warmth-aware verbs in `rpg_social_commands.py` (the 13 above + the existing $hug / $high_five / $fistbump dispatchers) all delegate to `dispatch_expressive_verb`. $greet keeps its inline silhouette-too-far check — the pending-silhouette responder is excluded from the default chain (`include_silhouette=False`) because silhouettes are at distance, not addressable. Self-directed verbs ($pose / $cheer / $cry / $bow) stay on the cog's local `_dispatch_self_directed` — they don't need entity routing. Active world verbs ($light / $feed / $touch) opt into `require_target=True` so a bare `$touch` prompts "at what?" rather than firing a self-directed beat.
- **Per-entity `handle_verb` adapters.** `StaticObjectPlugin.handle_verb` wraps `on_verb` with the `SUPPORTED_VERBS` gate; `Creature.handle_verb` wraps `on_social`; `Player.handle_verb` does the dead-target dispatch (corpse $hug routes through `on_hugged`; other warmth-aware verbs go to `_DEAD_TARGET_FLAVOR`) and the live-target warmth render via `_render_social`; `PasserbyPlugin.handle_verb` absorbs all the side-effect chain (`mark_encounter`, `apply_verb_credits`/`apply_greet_credits`, first-greet `mark_acquainted`, ACQUAINTANCE_CUE concatenation). Side effects only fire when the responder claims the verb (return ≠ `None`) — no warmth-credit or acquaintance leakage when falling through.
- **Deletions.** `_dispatch_two_actor_social`, `_maybe_route_to_passerby_social`, `_maybe_route_to_monster_social`, `_maybe_route_to_silhouette_too_far`, `_extract_target_token`, and the `_BOT_MENTION_REPLIES` dict in the social cog are gone. The world cog's command bodies shrank from ~30-line dispatch branches to single `await dispatch_expressive_verb(...)` calls.
- **Bot-mention identity by `.id`, not `is`.** Initially used `mention is bot_user`; first live `$glare @bot` returned "the mention didn't catch" because Discord delivers @-mentions as fresh `Member` objects each message, never `is`-equal to the cached `ctx.bot.user`. Switched to `mention.id == bot_user.id`. Bot-mention test rewired to use distinct mock objects with matching `.id` so a regression to `is`-comparison would actually fail.
- **Tests.** Existing tests rewritten to exercise the new entrypoints directly (`npc.handle_verb`, `walk_token_chain`, `resolve_passerby`) rather than the deleted cog helpers; mention-path tests now patch `helpers/verb_dispatch.Dispatcher` where the unified dispatcher lands. Full suite: 4419 passed.

### 2026-05-04 — Passerby warmth-credits: per-verb growth, witnessed-kill bumps, decay

Warmth was a five-tier enum with no path for upward movement — only `degrade_warmth` (via `flee_from_attack`) and the admin `$warmth set` override mutated it in gameplay. Players could never *earn* warmth from NPCs, even by greeting + hugging + protecting them. This adds the missing path.

- **`warmth_credits: int`** field on `PasserbyState` (range conceptually `[-100, +100]`). The `Warmth` tier is now derived from credits via the band table: HOT ≥40, WARM ≥20, NEUTRAL -19..19, COOL ≤-20, COLD ≤-40. The tier stays the player-facing concept; credits are the bookkeeping. `set_warmth` / `degrade_warmth` / `promote_warmth` reimplemented atop the credits model — they land at the destination tier's midpoint so subsequent fine-grained changes have headroom in either direction.
- **Per-verb-per-visit credit accumulation.** When a player uses a warmth-aware social verb on a present passerby: HOT/WARM verbs (hug, comfort) = +5; NEUTRAL verbs (wave, nod, salute, fistbump, wink, high_five) = +2; COOL verbs (poke, tickle) = -2; COLD verbs (glare, shank, taunt) = -5. Each verb counts at most once per visit (the same `$hug` twice in one wagoneer-stay yields one bump, not two), but different verbs each count independently. `$greet` is a special-case +5 once per visit (treated as a warm verb in its own dedup bucket). The mechanics are wired for all of the above, but per-NPC `SOCIAL_REACTIONS` pools currently only cover `$greet` / `$wave` / `$nod` — verbs without a per-NPC reaction pool early-return before the credit path, so today's actual warmth-shifting verb set is `greet` / `wave` / `nod` (max +9 per visit from social). Other verbs activate as each NPC's reaction pools get authored.
- **Queued-write race fix.** Per-call mutators (`mark_encounter`, `mark_acquainted`, `apply_verb_credits`, `apply_greet_credits`, `witness_kill`) used to each issue a full-slice `$set` write. Consecutive helpers in the same code path (e.g. `$greet` chains all three) read stale collection state because the `DoubleBuffer` queue hadn't drained between them, then their writes overwrote each other's fields when the queue eventually flushed. Refactored to per-field `$set` (with `$addToSet` for the visit-verb buckets) — non-overlapping fields don't clobber each other regardless of drain order. Lifecycle hooks (`mark_visit_arrival` / `mark_visit_depart` / `set_warmth` / `degrade_warmth` / `promote_warmth`) still write the full slice — they're coherent-snapshot mutations that don't have the race. Three regression tests in `TestQueuedWriteIndependence` pin the contract.
- **Silhouette-approach narration ordering fix.** Pending-silhouette NPCs (those who arrived during active combat) used to dispatch their post-combat approach line via `Dispatcher.add` directly inside `_drain_passerby_silhouette` — landing the line BEFORE the round narration the caller dispatched afterwards via `RoundOutput.render()`. The shepherd extolling the victory before players saw the killing blow narrate. Fixed: `_drain_passerby_silhouette` now returns the line, and `_finalize_combat` prepends it to `announce` so it rides along with the loot-hint slot in the round output. Order is now: round narration → death/flee narration → silhouette approach → loot prompt.
- **Passerby-name fuzzy matching fix.** `$greet herba` against the herbalist used to fail because the matchers in `_maybe_route_to_passerby_social`, `_maybe_route_to_silhouette_too_far`, and `_maybe_flee_passerby` (rpg_user_commands.py) all checked `candidate in content` or required exact-equal-to-stem matches. Refactored to follow the project's resolver pattern: new `PasserbyPlugin.matches_token` uses the canonical `fuzzy_match` (exact → prefix → substring → typo, against `name` + class stem + `ALIASES`); new `resolve_passerby` / `resolve_pending_silhouette` in `helpers/resolvers.py` mirror `resolve_active_monster`'s single-target shape; new `PasserbyConverter` / `PendingSilhouetteConverter` in `helpers/converters.py` for use as command-arg annotations. All three cog routing helpers call into the resolvers — one fuzzy contract, three call sites. Now `$greet herba`, `$wave wago`, `$nod shep`, `$kill wagoneer-typo` all resolve as expected.
- **Witnessed-kill credits.** When a Present passerby (NOT silhouette — silhouettes are too far to recognize details) is in the clearing and combat ends in a monster kill, every combatant in `self.looters` earns credits with the NPC. Non-passive monster: +10 ("you protected the clearing"). Passive monster (`AggressionLevels.PASSIVE`): default -10 ("most people don't want to watch you slaughter helpless things"), with per-NPC `PASSIVE_KILL_PENALTY` overrides.
- **Per-NPC `PASSIVE_KILL_PENALTY` map** on `PasserbyPlugin`, keyed by monster filename stem with `"*"` wildcard fallback. Value is either a numeric credit delta OR the literal `"tier_drop"` sentinel that drops one full warmth tier (lands at the top of the next-colder tier — e.g., HOT credits=60 → WARM credits=39; floors at COLD). Shepherd's = `{"sheep": "tier_drop"}` — kill a sheep in front of him, lose a tier. Other NPCs may add their own values over time (an herbalist might tier-drop on rare-flora destruction; a wagoneer might shrug at most passives).
- **Per-visit one-tier-shift clamp.** Captured `tier_at_visit_start` on every NPC arrival; per-visit credit changes can't push the player more than one tier up or down from their starting tier in a single visit, even if many positive or negative events stack. Prevents "$greet + 4 warm verbs + 3 monster kills = NEUTRAL → HOT in one visit." Tier-drop bypasses the clamp because it's a punctual consequence, not a slow accumulation.
- **Per-missed-visit decay — pulls TOWARD 0, not toward COLD.** When a known passerby visits a player and the player doesn't interact at all that visit (no verbs, no witnessed kills), credits step 1 toward 0 on depart: positive credits drop, negative credits rise, credits at 0 don't move. Decay floors at the NEUTRAL center — absence pulls a relationship to neutral and stops there, never crosses. Time-as-leveler, not time-as-cooler: hostile relationships also soften with absence (you're not actively building grudges with someone who isn't there). Players who've never interacted with an NPC have no slice and don't accumulate decay until they have a relationship to lose. `flee_from_attack` skips the decay (`apply_decay=False`) so bystanders don't get punished when another player attacks the NPC.
- **Visit lifecycle hooks** in `spawn.py`: `_try_mark_arrival` fires on Idle→Present (`attempt_spawn`) and Pending→Present (`drain_silhouette`), resetting per-visit tracking and capturing fresh `tier_at_visit_start` on every existing player slice for that NPC. `_try_mark_depart` fires on Present→Idle (`depart_passerby`) with decay, and on `flee_from_attack` without decay. Both wrappers swallow exceptions so a state-write failure can't block the spawn/depart narration.
- **Game.on_monster_death witness hook.** `_notify_passerby_witnessed_kill` fires before `_finalize_combat` (so `self.looters` is still in scope), iterating combatants and routing each to `witness_kill` with the monster's `aggression == PASSIVE` flag and its filename stem. Per-player exceptions caught individually so one bad write can't sabotage the whole post-death pipeline.
- **Legacy migration.** `PasserbyState.from_slice_doc` derives credits from a stored `warmth` tier midpoint when no `warmth_credits` field is present (HOT→50, WARM→25, NEUTRAL→0, COOL→-25, COLD→-50). LIVE's `passerby_state` collection is brand-new (V1 just shipped), so no real data to migrate; this defends against any TEST/dev docs that pre-date the credits field.
- **Tests.** ~50 new tests in `test_passerby_state.py` covering tier-from-credits boundaries, per-verb dedup, per-visit clamp (up and down), witness routing (passive/non-passive/wildcard/specific-stem), tier-drop semantics from each starting tier, decay-on-no-interaction, decay-skip on flee, and legacy slice migration. Existing schema-roundtrip tests updated to use `warmth_credits` instead of `warmth=` (which is now derived).

### 2026-05-03 — Passerby V1 runtime wiring + state schema restructure (LIVE)

Third passerby commit. The dormant code from the prior two foundation commits comes alive: spawn timer, combat-end silhouette drain, social-verb routing, $kill-flee path, $look extension, on_message overhear. State schema also restructured from per-triple to per-NPC docs because the bg-Vael session surfaced that the shepherd was tracking three separate documents per player and warmth wasn't actually being persisted on most writes.

- **`Game` integration.** New `passerby` / `pending_silhouette` fields. `set_passerby_timer` schedules `do_passerby_spawn` on a 15-45 min cadence (one-shot pattern, re-arms after fire — independent gate from `use_spawn_timer`, persisted via `use_passerby_timer` + `passerby_spawn_range` + `passerby_depart_after`). `_finalize_combat` calls `_drain_passerby_silhouette` BEFORE `end_combat` so the looters list is in scope for witness pickup; outcome detection prefers DEATH (any dead looter) over WON (`death` outcome with all looters alive) over FLED.
- **Social cog routing** (`rpg_social_commands.py`). New `_maybe_route_to_passerby_social` helper checked BEFORE the monster-route in `_dispatch_two_actor_social` so `$wave wagoneer` / `$nod wagoneer` land on the NPC. New `$greet <NPC>` command — explicit acquaintance event, marks acquainted via `"greet"` and dispatches the NPC's `ACQUAINTANCE_CUE_POOL` line (per-NPC voice — herbalist-as-jar-sealing vs Wren-writing-in-her-notebook). Bare `$wave` keeps self-directed; `$wave <unknown-text>` falls back to "waves at no one in particular" instead of silently dropping. `$greet <silhouette>` returns "too far off across the clearing to hear" instead of the misleading "introduces to nobody". `on_message` listener invokes `overhear_mentions` and dispatches the cue per newly-acquainted player. Set-intersect with the player roster bounds the overhear loop so a 50-mention message can't trigger 50 sequential Mongo round-trips on the event loop.
- **`$kill <passerby>`** (`rpg_user_commands.py`). New `_maybe_flee_passerby` helper called BEFORE the no-monster early return so a player can attack an NPC in a quiet window. Routes to `flee_from_attack`. Confirm-gate when the attacker is mid-combat with a different monster — emits a "type `$kill <npc> yes` to do it anyway" prompt rather than silently fleeing the NPC and burning a warmth tier.
- **`$look` extension** (`rpg_info_commands.py`). Bare `$look` now appends a present-passerby line (or pending-silhouette "watches from distance" line) after the time string. NPC-specific `LOOK_LINE` overrides; default fallback handles named (Wren) and article-using (wagoneer/herbalist/shepherd) NPCs.
- **Admin cog** (`rpg_admin_commands.py`). `$rp passersby` reload path. `$spawn` group restructured to `monster` / `passerby` action verbs at top level + `config monster` / `config passerby` sub-groups for tuning (min/max/duration/loot-monster-only/set on|off). New `$spawn passerby [name]` force-spawn (fuzzy resolve, random fallback) and `$spawn depart` for the present NPC. Backward-compatible old paths — the move is muscle-memory only.
- **State schema restructure** (`creatures/passersby/state.py`). One doc per `(channel_id, npc_stem)` instead of per-triple; players carried as a `players: {<id_str>: <slice>}` sub-mapping. Atomic per-player writes via dotted-path `$set: {f"players.{pid}": <full_slice>}` so concurrent writes for different players (Caels waving while Vael greets) don't clobber each other. Every mutator now writes the FULL slice every time — fixes the bug where `mark_encounter` / `mark_acquainted` only persisted partial fields and warmth never made it to disk for most players. Inspection-via-Compass now shows the full state. Migration: drop `passerby_state` collection on deploy (TEST done; LIVE pending).
- **Parser-token sweep across all 4 NPCs.** 12+ bare `@1np` (noun-possessive) usages where the noun never followed produced ungrammatical "the traveler's" hanging in air at COOL/COLD social tiers. Replaced with `@1d` (definite article + name). Also fixed two pre-existing convention bugs in `FLEE_FROM_ATTACK_POOL` where `@1`/`@2` were swapped against the rendering call site (`render_actor_npc(line, attacker, npc)` passes attacker as @1, not the wagoneer's outdated comment). Plus the herbalist's *"Hold it again next time"* line — read as wagoneer coaching, not herbalist aphorism — replaced with *"It will hold longer next, or it won't. The dirt watches either way."*
- **`ACQUAINTANCE_CUE_POOL` added per NPC.** Italicized one-liner dispatched on the moment of name-learning (greet OR osmosis-flip OR overheard-mention) so the player sees evidence the NPC now knows their name — without this the bg-Vael shepherd-test couldn't tell whether `$greet` had taken (low-NAMING_BIAS NPCs would still surface as "traveler" 70-85% of subsequent renders).
- **Defensive cleanups in `spawn.py`.** `getattr(game, "channel_id", 0)` defaults stripped — would have collapsed cross-channel state onto channel_id=0 on AttributeError. Same for `witness_id` / `attacker_id`. Real Game always has these; the defensive defaults masked real bugs.
- **Tests.** 30 in `test_passerby_state.py` (rewritten for new schema, +new persistence-guarantee tests), 36 in `test_passerby_integration.py` (new — covers `_finalize_combat` outcome detection, social routing with osmosis flip, $kill confirm gate, $look line, ACQUAINTANCE_CUE on first learn, overhear list-return, silhouette-too-far), 16 in `test_rpg_admin_spawn_passerby.py` (new — force-spawn / depart / config tunables). Total suite: 4235 → 4369.

### 2026-05-03 — Static objects: campfire + stone field + Area-hosted ambience + MENDHOLM shortname + journal --stdin

Persistent room features Caels asked for after the passerby work — things that live in the clearing rather than spawning into it. Plus the supporting Area refactor, MENDHOLM as a first-class env shortname for Vael's bg session, and a journal-tool fix for the Windows .bat truncation that ate her multi-paragraph entries.

- **`StaticObjectPlugin` family** at `caldanai/lib/rpg/world/objects/`. Same plugin/discovery/registry shape as `MonsterPlugin`/`PasserbyPlugin`. Per-instance state, `SUPPORTED_VERBS` opt-in, `on_verb(verb, game, actor, *args)` dispatch hook, `get_look_line` / `maybe_emit_ambience` / `on_weather_change` hooks. Auto-instantiated per-Area at game-load time.
- **Campfire** (`world/objects/campfire.py`). `FireState` enum (LIT / EMBERS / OUT) + fuel tracking (default 30 units, +5 per stick, weather-modulated burn rate). All five verbs: `$light` (refuses without fuel from out, free from embers, flavored already-lit case), `$feed campfire [fuel]` (implicit picks most-abundant fuel from inventory, explicit takes the fuel name; consumes via `Player.take_item`; state-aware response — embers-recovery vs eager vs calm), `$gaze` (state + time-of-day pool), `$touch` (1d4 fire damage routed through `apply_damage` with a real target part), `$listen` (crackle/embers/silence per state). Weather reactivity: light rain hisses but holds, heavy rain schedules a 2-tick delayed extinguish, gusty wind throws ember-shower ambience. Defaults LIT at game start ("the world is alive when you arrive").
- **`$touch` damage routing hardened** (playtest-driven, Caels 2026-05-04). Three issues caught in the OOC channel: (1) corpses could keep $touching the fire with no death gate, (2) once both hands were destroyed, `find_parts("hand")` returned `[]` and damage spilled silently to core HP via the no-target legacy path — Caels burned from 20 → 0 with no narration of what was happening, (3) `apply_damage`'s return string (gear-drop + alive→dead death tail) was being discarded so the kill itself was invisible. Fix: dead-actor check at top of `_on_touch`, `_pick_touch_target` walks hand → arm so a stump still takes the burn (no living arm = symbolic touch with no damage), `apply_damage` return value appended, plus a per-call injury-level diff that emits a one-line worsen beat ("@2np hand blackens past use — given over to the fire") when the targeted part visibly degrades. Five regression tests in `test_world_static_objects.py` pin each branch.
- **Stone field** (`world/objects/stone_field.py`). Vael's invented landmark made real — every walked-off golem in her interior framing was *"a stone, waiting to wake"*; after enough avoidance the clearing accumulated *"a quiet stone field"* in her narration. V1 ships it as a present-since-game-start landmark with three sensory verbs (`$gaze` / `$touch` / `$listen` — silent stones, dim glyphs at night). Grow-with-each-golem-walk beat reserved for a follow-up.
- **`do_ambience` Game→Area refactor.** `Area.collect_ambience(game)` walks the area's own `AMBIENCE_POOL` (meadow wildlife moved out of `Game.do_ambience` inline) AND each present static object's `maybe_emit_ambience` per-tick. Per-object cadence — campfire is more vocal at night than during the day; stone field is rare. `Game.do_ambience` is now an orchestrator that gates on `enable_ambience` flag, collects emissions, combines via `combine_ambience_lines`, dispatches as one message. **Combine heuristic:** 1 line passthrough; 2 lines spliced as "{A_no_period}, as {B_decapped}"; 3+ Oxford-comma chain. Skips combining for lines containing escaped dialogue (reads awkwardly mid-sentence). Decap rule respects `known_names` (player roster) so "Wren writes" stays capitalized when spliced. Forward-compat with threaded dungeons because Area owns its own object set.
- **Weather hook wiring.** `WeatherDaemon` gains a `transition_listeners` callback registry; daemon's tick fans out `(old_patterns, new_patterns, old_severities, new_severities)` to every registered listener after each state roll. `Game.__init__` AND `Game.from_dict` register `_on_weather_change` (the from_dict registration is critical — `from_dict` REPLACES `game.weather` with a fresh daemon to bind persisted state, so the listener registered in `__init__` would have been on the orphaned daemon; without re-registering here every load-path game silently lost weather-reactive object behavior). The Game-level listener fans out to `room0.static_objects.values()` and dispatches collected narration lines.
- **`rpg_world_commands.py` cog.** Five generic-verb commands: `$light`, `$feed`, `$gaze`, `$touch`, `$listen`. Resolution chain puts static objects LAST (omnipresent matches lose to transient ones — design rule from Caels). Sensory verbs (`$gaze` / `$listen`) bare-no-target → self-directed pool ("Caels gazes into the middle distance"); active verbs require a target. `$feed campfire [fuel]` extracts the fuel arg from the trailing token so explicit fuel-name forms work alongside the implicit "find most abundant" path.
- **`StaticObjectConverter`** in `helpers/converters.py`. Fuzzy resolves a token against the present room's static objects via `Area.find_static_object`. Documented to match LAST in cogs that mix entity types — same precedence rule as the cog's resolution chain.
- **`$look` extension.** Bare `$look` now appends weather sentence (from `WeatherDaemon.describe()`) and each present static object's `get_look_line` after the time string. Order: scenery → time → weather → static objects → passerby.
- **MENDHOLM shortname + TEST→OOC remap.** New `MENDHOLM` entry in `TAIL_ENVS` → `(TEST_DB_NAME, port 8767, MENDHOLM_CHANNEL_ID)`. Same DB as TEST, but pre-resolves the channel via the env var so Vael's bg session never lands on the multi-game picker that bare TEST used to hit. `tools.tail_channel MENDHOLM --gateway` and `bot_player send --mendholm "..."` both route to her world directly. The bare `TEST` shortname is now bound to `OOC_CHANNEL_ID` — the picker was muscle-memory-annoying and the OOC channel is the most natural backchannel target — so `tail_channel TEST` and `tail_peek TEST` both go straight to OOC. New `resolve_implicit_channel_id` helper in `tools/_common.py`; `--mendholm` flag added alongside `--ooc`/`--obs` on bot_player send/react/edit/thread (mutually exclusive). Loud failure if the env var is unset rather than silent picker fallback.
- **Journal `--stdin` / `--file`** (`tools/journal.py`). Multi-paragraph entries failed silently to first-paragraph-only because Vael's `vael-python.bat` ends with `python.exe %*` — `%*` re-splits args through cmd.exe which collapses embedded newlines in quoted strings. New `--stdin` reads the full content from stdin via heredoc; `--file` reads from a path. Positional arg path remains for short single-paragraph entries. Stderr log at submission time (`[journal] received N chars, posting in M chunk(s)`) so the operator sees immediately if upstream truncated. Same flags added to `journal edit` for replacement-from-stdin recovery. The one-shot `tools/journal_salvage.py` recovery script is REMOVED — its job is done (Vael's truncated entries were salvaged, the bug is fixed at the source). 10 new tests pin the resolution contract.
- **Misc fixes folded in.** Inventory-iteration infinite-loop in campfire's `_collect_fuel_items` (Inventory has `__getitem__` but no `__iter__`, Python's int-fallback loops forever — use `inventory.all()`). Daytime LIT campfire look-line was "crackles low" which read as state-affected; clarified to "burns steadily ... flames modest against the daylight" so it doesn't read as rain-impacted. `$rp objects` admin reload path. `Inventory` API consistency check via mock-spec regression test.
- **Tests.** 54 in `test_world_static_objects.py` (new — plugin discovery, area integration, ambience-combine heuristic, campfire state machine + verbs + weather reactivity, stone field, world cog dispatch, inventory-iteration regression, plus 4 dedicated `$touch` safety regressions). Total suite: 4369 → 4374.

### 2026-05-03 — Passerby pure-logic layer: rendering / state / spawn-pipeline / SpawnTimeMixin (still dormant)

Second passerby foundation chunk. Pure-logic helpers — rendering layer, MongoDB state persistence, spawn-pipeline state machine, time-of-day gating mixin. All testable in isolation. Still nothing wired into bot startup; runtime integration (Game timer, combat-end hook, cog commands, `$look` extension) lands in the next commit.

- **Render layer** (`creatures/passersby/rendering.py`). `StrangerActor` wraps a real player so `@1d` renders as `"the traveler"` while pronouns / user_id / verb-agreement attrs proxy through unchanged (NPC never mis-genders an unacquainted player; mention tags still ping the real user). Three render helpers map to the three pool shapes: `render_npc_only` (arrival / ambient / departure / silhouette — no actor-side wrap), `render_combat_witness` (NPC + witness, witness gets the wrap), `render_actor_npc` (player + NPC, player gets the wrap). `_maybe_unwrap` rolls once per line so a single rendered string is internally consistent — never `"Caels ... the traveler's"` mid-sentence.
- **State persistence** (`creatures/passersby/state.py`). New `passerby_state` collection: one document per `(channel_id, npc_stem, player_id)` triple carrying `warmth` / `acquainted` / `acquainted_via` / `met_count` / `first_met` / `last_seen`. Helpers: `mark_encounter` (auto-promotes to acquainted via osmosis at `met_count >= 3` with NEUTRAL+ warmth), `mark_acquainted` (idempotent — keeps the FIRST via-tag as canon), `degrade_warmth` / `promote_warmth` (ladder-stepping with floor/ceiling), `set_warmth`. Three acquaintance learning events: `"greet"` (explicit), `"mention"` (overheard via @mention from another in-channel player), `"osmosis"` (passive accumulation).
- **Spawn pipeline** (`creatures/passersby/spawn.py`). Pure-function state machine taking a duck-typed game object — Idle ↔ Present ↔ Pending Silhouette transitions handled by `attempt_spawn` / `drain_silhouette` / `depart_passerby` / `flee_from_attack`. Each function modifies `game.passerby` / `game.pending_silhouette` directly. Combat-active gate uses `is_combat_active` (alive monster + non-empty combatants). Combat-end hook drains pending silhouettes with outcome-aware reaction pools (won / fled / death). `flee_from_attack` reads PRE-degrade warmth so the flee line uses the attacker's name if known, then degrades after.
- **`overhear_mentions`** — Discord-message hook. When a player posts in-channel and a passerby is present (or in silhouette), scans `message.mentions` and marks each mentioned game player as acquainted via "mention". Filtered through `game.player_manager.players` so bystander @mentions of non-game-Discord-users don't leak acquaintance.
- **`SpawnTimeMixin`** (`helpers/spawn_time.py`). Composable time-of-day spawn gating. Shared by `MonsterPlugin` (existing instance-level pattern) and `PasserbyPlugin` (new class-level declaration). Class-level `time_partition` default = `CATHEMERAL` so composing the mixin doesn't narrow spawn windows unintentionally. `is_active_at(time)` instance accessor + `filter_active(candidates, time)` classmethod for filtering plugin classes pre-instantiation. `pick_npc` honors the gate so Wren doesn't run errands at midnight and the wagoneer doesn't cart at 02:00.
- **Per-NPC time partitions:**
  - **Wagoneer** — `DIURNAL` (dawn through evening; "rolls in before sundown")
  - **Shepherd** — `DIURNAL` (sheep-tending hours)
  - **Wren** — `DIURNAL` (kid expected home by sundown)
  - **Herbalist** — `DIURNAL | CREPUSCULAR` (gathers in good light, walks the verges at dusk for herbs that only show themselves then)
- **`PasserbyPlugin.__init__`** added to convert class-level pronoun strings to instance-level `Dict[Pronouns, str]` at instantiation, mirroring `Creature.__init__`'s pattern. Without this the parser's `actor.pronouns[Pronouns.X]` access would fail on the unparsed string.
- 82 new tests across `test_passerby_rendering.py` / `test_passerby_state.py` / `test_passerby_spawn.py` covering: tier-non-overlap of pool selection, StrangerActor proxy semantics, NAMING_BIAS bias-rolling logic, warmth ladder floor / ceiling, osmosis threshold, idempotent acquaintance, time-of-day filter inclusion / exclusion, @mention learning, flee-degrade-after-render ordering. Total suite: 4153 → 4235.
...
- **`FuzzyResult` + `fuzzy_match`** (`caldanai/lib/rpg/helpers/fuzzy.py`). Tiered structure carrying one list per pass-tier (exact / prefix / substring / edit_distance), with `.tightest` returning the strictest non-empty tier (most callers) and `.all` returning the union across tiers (autocomplete-style). Individual tier access enables "did you mean..." UX. List-compatible dunders (`__bool__`, `__iter__`, `__eq__`-against-list) so existing call sites that treat it as a list keep working.
- **`helpers/resolvers.py`** + **`helpers/converters.py`**. Free `resolve_*` functions per domain (monster_class, active_monster, player, part, recipe) and discord.py `Converter` wrappers (Monster, MonsterClass, Player, Creature(prefer=), Part, Recipe, FuzzyMember). Lives under `helpers/` not `cogs/` — the cogs directory is the bot's auto-loader and expects `setup()` per module.
- **Behavior wins:** `$haunt hyd` against hexed hydra (was exact-only); `$creature destroy hyd head.1` (was substring `in`); `$health Cael` / `$health zzxx` (new fuzzy-player path, with stripped-`@` for malformed mentions); `$warmth set hug warm Cael` (FuzzyMemberConverter on `who`); `$craft jerkin` / `$craft Jerkin` / `$craft leather` (fuzzy + ambiguity-list + group case-insensitive); inventory queries (`$equip`, `$stow`) gain prefix-wins + typo tolerance via `Inventory._filter_by_name`.
- **Conflict-check tightening.** `_strip_leading_monster_token`'s peel guard previously used `monster.find_parts` (which now runs edit-distance) — `$kill hex head.1` against a hexed hydra was blocked because `find_parts("hex")` matched `head.*` via a 1-edit collision (`"hex"` is one edit from `"he"`, the 2-char prefix of `"head"`). Now uses a stricter callable that runs `fuzzy_match` with `edit_distance=False`, so clear monster-word prefixes peel cleanly while single-letter part shortcuts (`h`, `t`) still defend the part interpretation.
- **`Optional[FuzzyMemberConverter]` footgun fix.** discord.py swallows `BadArgument` from converters when the param is wrapped in `Optional[]`, silently falling back to `None`. `$warmth set hug warm nobodyxyz` was setting defaults instead of erroring. Drop `Optional[]`, keep the `= None` default — friendly error fires, bare-call (no `who` arg) still works.
- **`tools/test_fuzzy_paths.py`** — sectioned checklist driver for manual playtest. Routes through `bot_player` against the OOC channel by default; `--combat` for the regular TEST channel. `--section` filter, `--dry-run`, `--list`, `--pause-between-sections`. Each section announces preconditions in-channel as a markdown header so the log self-documents.
- 63 new tests across `test_converters.py`, `test_fuzzy_helpers.py`, and `test_kill_command_grammar.py` (regression pin for the `hex` peel bug).

### 2026-05-02 — `$pray` 17-19 crash on small-injury heal targets

Pray rolled 17-19 with the picked heal target's injury surface ≤ 4 (light bruise + small body gap — the common state at end-of-fight) crashed with `TypeError: unsupported operand type(s) for +: NoneType + int`, swallowing the entire prayer. `quarter = max(1, ceil(surface/4))` produced 1; `Dice.quick_roll("1d1")` returned `None` because `Dice.from_ndn` rejects `sides < 2` by design; `None + quarter * (d20 - 17)` blew up the heal arithmetic. Three other places in the codebase already shortcut `1d1` (`_q7_absorbed_dn` in creatures, `general_commands` parse path, player roll guard); the heal path just missed it. One-liner fix: `1 if quarter <= 1 else Dice.quick_roll(...)`. Regression test in `test_pray.py::TestNat17To19SmallInjuryQuarterOne` exercises real `Dice.quick_roll` (no mock) so the same omission can't slip past the suite again.
...
### 2026-05-02 — `--obs` channel shortcut

`--obs` shortcut added to `bot_player` (send/react/edit) and `tail_channel`. New `OBSERVATIONS_CHANNEL_ID` env var holds the snowflake for an observations channel; `--obs` is sugar for `--channel-id $OBSERVATIONS_CHANNEL_ID`. Mutually exclusive with `--ooc`. Tests in `test_tools_bot_player.py`.
...
- **OOC engineering channel tooling.** New `OOC_CHANNEL_ID` env var (gitignored) holds the snowflake for the out-of-character engineering playtest channel. `bot_player send|react|edit --ooc` and `tail_channel --ooc` are shortcuts for `--channel-id $OOC_CHANNEL_ID`. The default DB lookup in `bot_player` excludes the OOC channel via `$ne` so existing flows (bg Vael's no-flag `bot_player send`) keep landing on the original Vael-facing channel. bg Vael's workspace is untouched — her `.env`, `CLAUDE.md`, and memory dir have zero OOC references; her worldview stays sealed.

### 2026-05-02 — Crafting XP rebalance: per-attempt bump + level-scaled successes + compound-damage hint fix

- **Crafting per-attempt XP bumped and level-scaled.** `RecipePlugin.xp_reward_success` 5 → 20, `xp_reward_failure` 2 → 5 as the per-attempt base. New `Player.gain_craft_experience(skill, base_xp, succeeded)` adds `floor(10 * sqrt(level))` on top of the success base — mirrors the sqrt-of-level shape combat uses (`Player.gain_skill_experience`) but with 2× the multiplier since crafting is material-gated. Failure is flat (no level scaling) — same shape as combat misses. Per-recipe overrides still work; recipes can set their own base if a special case warrants. Math: leatherworking 1→2 falls from ~286 attempts to ~50 at level 1 (Celowin's 2026-05-02 community feedback). Higher levels stay manageable instead of blowing up against the quadratic level threshold (L4→L5 lands ~105 attempts vs the prior unbounded grind). Cog now displays the actual XP granted per craft, so high-skill players see their progression accelerate. `tests/test_player.py::TestGainCraftExperience` covers L1, L4, level-20 cap, unknown-skill init, and failure-no-scale.
- **Compound-damage hint chooses the right component.** `Creature.get_hit_narration` previously returned the FIRST matching `HIT_NARRATIONS` entry in MRO declaration order — for an ice axe (`SLASHING | WATER`) vs a golem (slashing 0.5×, water 1.25×), this told the player they were hitting a resistance even though water actually drove the multiplier. Now ranks matching components by `|trait_multiplier - 1.0|` and narrates the most-extreme matchup. Three regression tests in `test_classifications.py` (golem + ice axe → slashing wins as more-extreme resistance; vampire + holy mace → light wins despite later declaration; single-type unchanged).
