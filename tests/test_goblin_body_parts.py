"""Tests for the Phase 1.D item 4.1 Goblin body parts migration.

Item 4.1 is the first migration of an existing monster to the body
parts system. The goblin gains a standard humanoid composition
(1 head + 1 torso + 2 arms + 2 legs) but behaves identically at full
health to its pre-migration self:
...
- ``Creature.apply_damage(amount, target_part=None)`` routes through
  ...
  goblin's main HP directly.
- At ``InjuryLevels.NONE`` every part's ``debuffs`` lookup returns 0,
  so ``get_defense`` / ``get_dodge`` are unchanged from the base
  attributes.
- ``get_attack_sources`` is NOT overridden — the goblin still attacks
  with a single ``NaturalAttackSource`` derived from ``self.attack``,
  not per-limb.

The tests in this module pin those invariants plus the composition
shape, so items 4.2 (Bandit) and 4.3 (Giant) can reuse the same
...
class TestGoblinCriticalParts:
    def test_head_is_critical(self):
        """A decapitated goblin dies. Head inherits ``is_critical=True``
        from ``HeadPlugin``; the goblin does NOT override it."""
        g = Goblin()
    ...
    def test_torso_is_critical(self):
        """Critical torso: destruction kills the goblin via the standard
...
class TestGoblinLegacyDamagePath:
    ...
    def test_goblin_get_attack_sources_is_single_source(self):
        """The goblin is NOT overriding ``get_attack_sources``. It
...
# ---------------------------------------------------------------------------
# Sanity — existing goblin fields unchanged
# ---------------------------------------------------------------------------


class TestGoblinSanityUnchanged:
    def test_name_is_goblin(self):
        g = Goblin()
    ...
    def test_loot_table_contains_expected_items(self):
        ...
        g = Goblin()
        assert "stick" in g.loot
        assert "rock" in g.loot
        assert "torch" in g.loot
    ...
    def test_per_instance_parts_are_independent(self):
        """Two fresh goblins must not share the same body part
