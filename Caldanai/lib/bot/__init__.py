...
from discord.ext.commands import CommandNotFound, BadArgument, CommandOnCooldown, MissingRequiredArgument, when_mentioned_or, MissingPermissions
...
class Bot(BotBase):
	...
	def discover_cogs(self):
		self.COGS = [ path.split("/")[-1][:-3] for path in glob("./Caldanai/lib/cogs/*.py") ]
	...
	async def on_error(self, err, *args, **kwargs):
		if err == 'on_command_error':
			await args[0].send('Something went wonky.')
	...
	async def on_command_error(self, ctx, exc):
		...
		elif isinstance(exc, (Forbidden, MissingPermissions)):
