...
class MongoDatabase(Observer, Subject):
	...
	async def _poll_for_updates(self):
		"""Requests any updates from observers that need to be pushed to the database."""
		await self.notify(DatabaseMessages.REQUEST_UPDATE)
	...
	def delete_game(self, game_id):
		...
		self._queues["games"].put(DeleteOne({'guild_id': game_id}))
		self._queues["players"].put(DeleteMany({'guild_id': game_id}))
